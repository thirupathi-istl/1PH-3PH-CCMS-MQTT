...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['D_ID']) && isset($_POST['ON_OFF_MODE']) && isset($_POST['TIME'])) {
	...
	$mode = htmlspecialchars(mysqli_real_escape_string($conn, $_POST['ON_OFF_MODE']));
	$time = htmlspecialchars(mysqli_real_escape_string($conn, $_POST['TIME']));

	$device_ids_array = explode(",", $device_ids);

	$sql = "SELECT on_off_control FROM user_permissions WHERE login_id = ?";
	...
	if ($permission_check == 1) {
		foreach ($device_ids_array as $device_id) {
			...
			if ($insert_stmt) {
				...
				$on_off_activity="Initiated lights ". $mode ;
